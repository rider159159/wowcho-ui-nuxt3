  ...
  <div class="mx-auto flex flex-col items-center">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6 items-center">
          <div class="flex flex-col">
            ...
            <div class="flex justify-center md:justify-start">
              <NuxtLink to="/proposals" class="rounded-5xl transition duration-500 px-12 md:px-8 py-3 border bg-brand-1 text-white text-center border-2 border-brand-1 leading-h6 md:leading-h5 hover:bg-brand-2 hover:border-brand-2">我想贊助</NuxtLink>
            </div>
          </div>
          <div>
            <img src="/cheers.png">
    ...
    <div class="w-[calc(100vw-24px)] md:w-4/5 md:max-w-324 h-full py-8 pb-20">
      <div class="w-full flex justify-between items-center mb-10">
        ...
        <NuxtLink to="/proposals" class="hidden text-h5 text-gray-2 lg:flex gap-10px cursor-pointer items-center">
          <p>查看更多</p>
    ...
    <div class="w-full py-8 md:py-20 flex flex-col gap-8 items-center relative bg-brand-4 overflow-hidden">
      ...
      <ul class="hidden md:flex">
        <li v-for="category in categories" :key="category.path" class="border-line md:text-h5 leading-h6 md:leading-h4 text-gray-2 font-medium px-6">
          <NuxtLink :to="`/proposals?category=${category.path}`" class="block relative z-1">{{ category.name }}</NuxtLink>
        </li>
      </ul>
      <div class="flex flex-col md:hidden gap-7">
        <ul class="flex">
          <li v-for="category in categories.slice(0,4)" :key="category.path" class="border-line md:text-h4 leading-h6 md:leading-h4 text-gray-2 font-medium px-6">
            <NuxtLink :to="`/proposals?category=${category.path}`" class="block relative z-1">
              {{ category.name }}
            </NuxtLink>
          </li>
        </ul>
    ...
    <div class="m-auto px-3 md:w-4/5 md:max-w-324">
      <div class="w-full md:max-w-324 px-3 md:px-0 pt-8 md:pt-20 flex flex-col justify-center items-center gap-6 md:gap-10 border-b-1 border-line">
        <div class="w-full flex justify-between items-center">
          ...
          <NuxtLink to="/proposals?order=3" class="hidden text-h5 text-gray-2 lg:flex gap-10px cursor-pointer items-center">
            <p>查看更多</p>
            <img class="w-4 h-4" src="/arrow.svg" >
          </NuxtLink>
        </div>
        <div class="w-full flex flex-col lg:flex-row justify-between gap-7 md:gap-6">
          <NuxtLink :to="`/proposal/${recentlyProposal.customizedUrl}`" v-for="recentlyProposal in recentlyProposalList" :key="recentlyProposal.id" class="w-full flex items-stretch">
            <CommonProductCard
        ...
        <NuxtLink to="/proposals?order=3" class="md:hidden leading-h6 text-gray-2 flex gap-10px cursor-pointer items-center mt-2 mb-8">
          <div>查看更多</div>
      ...
      <div class="w-full md:max-w-324 px-3 md:px-0 py-7 md:pt-14 flex flex-col justify-center items-center gap-6 md:gap-10">
        <div class="w-full flex justify-center lg:justify-between items-center">
          ...
          <NuxtLink to="/proposals?order=1" class="hidden text-h5 text-gray-2 lg:flex gap-10px cursor-pointer items-center">
            <p>查看更多</p>
            <img class="w-4 h-4" src="/arrow.svg" >
          </NuxtLink>
        </div>
        <div class="w-full flex flex-col lg:flex-row justify-between gap-7 md:gap-6">
          <NuxtLink :to="`/proposal/${hotProposal.customizedUrl}`" v-for="hotProposal in hotProposalList" :key="hotProposal.id" class="w-full flex items-stretch">
            <CommonProductCard
        ...
        <NuxtLink to="/proposals?order=1" class="md:hidden leading-h6 text-gray-2 flex gap-10px cursor-pointer items-center my-2">
          <div>查看更多</div>
          <img class="w-3 h-3" src="/arrow.svg" >
        </NuxtLink>
      </div>
